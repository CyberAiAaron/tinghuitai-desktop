...
module.exports=async function(req,res,u,{isLocal,settings,active,testModel}){
 ...
 const publicState={provider:c.LLM_PROVIDER||'',ready:!!(c.VOLC_APP_KEY&&c.VOLC_ACCESS_KEY&&(c.DEEPSEEK_API_KEY||c.LLM_PROVIDER)),asrConfigured:!!(c.VOLC_APP_KEY&&c.VOLC_ACCESS_KEY),modelConfigured:!!(c.DEEPSEEK_API_KEY||c.LLM_PROVIDER),base:c.LLM_BASE_URL,model:c.LLM_MODEL,resource:c.VOLC_RESOURCE_ID,archive:c.ARCHIVE_TARGET};
 ...
 try{const j=JSON.parse(body);for(const k of Object.keys(settings.defaults)){if(j[k]===undefined||['ARCHIVE_TARGET','THT_ARCHIVE_OWNER_ID','LLM_PROVIDER'].includes(k))continue;if(typeof j[k]!=='string'||j[k].length>4000||/[\r\n]/.test(j[k]))throw Error('字段格式不正确');if(['VOLC_APP_KEY','VOLC_ACCESS_KEY','DEEPSEEK_API_KEY'].includes(k)&&!j[k])continue;c[k]=j[k].trim();}
