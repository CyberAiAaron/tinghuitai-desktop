...
const defaults={PRESET_VERSION:'',ASR_PROVIDER:'',DEEPGRAM_API_KEY:'',MEMORY_PROJECTION_DIR:'',LLM_PROVIDER:'',VOLC_APP_KEY:'',VOLC_ACCESS_KEY:'',VOLC_RESOURCE_ID:'volc.seedasr.sauc.duration',DEEPSEEK_API_KEY:'',LLM_BASE_URL:'https://api.deepseek.com',LLM_MODEL:'deepseek-chat',LLM_MODEL_QUICK:'',LLM_MODEL_LIVE:'sonnet',LLM_MODEL_POST:'opus',LLM_LIVE_THINKING:'0',TRIAGE_CONTEXT_DELTA:'0',ARCHIVE_TARGET:'local',THT_ARCHIVE_OWNER_ID:'',AUDIO_RETENTION_DAYS:'30',JEV_API_KEY:'',JEV_GATE:'off',JEV_THRESHOLD:'0.5',JEV_MIN_GAP_MS:'2000',DECISION_BOARD_DIR:'',MEETING_PUSH:'off',MEETING_PUSH_SELF_NAMES:'',MEETING_PUSH_MIN_GAP_MS:'120000',MEETING_PUSH_TO:''};   // DECISION_BOARD_DIR：决策板夜间导出目录（app/decision-board.js，默认 <PROJECT_CONTEXT_DIR>/kb_backup）；MEETING_PUSH*：会中飞书提醒白名单推送（app/push-whitelist.js，THT-R4：默认 off = 零推送，分诊照跑）   // LLM_LIVE_THINKING：会中分诊（live 档）走 claude 命令行时的思考预算。默认不带思考（'0'；实测分诊 2,000 多输出 token 大半是思考，回放中位 15.2 s → 5.1 s、条目数没少），设 '1' 或更大恢复思考（值 = 预算上限），'' = 不干预沿用命令行默认   // TRIAGE_CONTEXT_DELTA：项目背景「hash 不变只给占位」，默认 '0' 关（claude -p 每次新进程，模型看不见「上一次」，占位那轮没有背景可对照），'1' 启用   // JEV_*：逐句门卫（app/jev-gate.js），JEV_GATE=on 且有密钥才调；密钥只在 settings.json（2026-09-22 Aaron 确认上云链路）   // AUDIO_RETENTION_DAYS：录音保留天数，0 = 不清理；文字永不删（2026-09-22 Aaron 定）
