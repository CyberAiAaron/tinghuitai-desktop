...
function sync({dataDir,outputDir,extraPendingDirs=[],liveSessions=[]}){
 ...
 const sourceRevision=hash(JSON.stringify(rows.map(r=>[r.s.id,r.s.title,r.s.topicTitle,r.s.start,r.s.end,r.s.summary,r.s.notes,r.s.highlights,r.s.transcript,r.s.todos,r.s.names,r.s.recoveryStatus]))+JSON.stringify(found.errors));
 ...
 for(const {s,file:source} of rows){const tr=s.transcript||[],hl=s.highlights||[];lines.push(`## ${clean(s.title||s.topicTitle||s.id,120)}`,`开始：${new Date(date(s.start)).toISOString()}；${s.end?'已结束':'未收到结束标记，内容可能仍在更新'}`,`完整原始记录：${source}`,`转写 ${tr.length} 段；要点 ${hl.length} 条；${s.summary?'已有总结（节选如下）':'尚无总结，以下仅供了解原话，不代表已完成分析'}`);if(s.names)lines.push('说话人：'+clean(JSON.stringify(s.names),600));if(s.todos?.length)lines.push('待办（最多20条）：',...s.todos.slice(0,20).map(t=>'- ['+(t.done?'x':' ')+'] '+clean(t.text||t.title,300)));if(s.recoveryStatus)lines.push('恢复状态：'+clean(s.recoveryStatus,100));if(s.summary)lines.push('',clean(s.summary,1600));if(s.notes)lines.push('本人笔记（节选）：'+clean(s.notes,600));if(hl.length)lines.push('最近要点（最多6条，非全场结论）：',...hl.slice(-6).map(x=>'- '+clean(x.text,260)));lines.push('最近原文（最多8段，每段最多240字符）：',...tr.slice(-8).map(x=>'- '+clean(x.text,240)),'');}
