...
// ===== 项目资料：设置项里配好的本机文件 =====
// PROJECT_CONTEXT_DIR + PROJECT_CONTEXT_FILES（相对通配）、PROJECT_FOCUS_FILES、FACT_SOURCE_FILES（绝对路径）。
const DEFAULT_CONTEXT = ['kb_reorg/*.md', 'kb_backup/决策板*.md', '.memory/MEMORY.md', '.memory/meeting-memory.md', '.memory/project-state.md', 'CLAUDE.md'];
const expand = p => String(p || '').replace(/^~(?=\/)/, process.env.HOME || '~');
// 小通配：只认路径段里的 *，够用且不引依赖。段里有 * 就 readdir 后按正则挑。
function globIn(base, pattern) {
  const parts = String(pattern).split('/').filter(x => x && x !== '.');
  if (parts.some(x => x === '..')) return [];
  let cur = [base];
  for (const seg of parts) {
    const next = [];
    for (const dir of cur) {
      if (!seg.includes('*')) { next.push(path.join(dir, seg)); continue; }
      const re = new RegExp('^' + seg.split('*').map(x => x.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')).join('.*') + '$');
      let names = [];
      try { names = fs.readdirSync(dir); } catch (e) { continue; }
      for (const n of names.sort()) if (re.test(n)) next.push(path.join(dir, n));
    }
    cur = next;
...
function contextFiles(env) {
  const out = [];
  const dir = String(env.PROJECT_CONTEXT_DIR || '').trim();
  if (dir) {
    const base = path.resolve(expand(dir));
    const pats = Array.isArray(env.PROJECT_CONTEXT_FILES) && env.PROJECT_CONTEXT_FILES.length ? env.PROJECT_CONTEXT_FILES : DEFAULT_CONTEXT;
    for (const pat of pats.slice(0, 20)) {
      if (typeof pat !== 'string' || pat.startsWith('/') || pat.includes('..')) continue;   // 只许在背景目录里面找
      const hits = globIn(base, pat);
      for (const f of (/kb_backup/.test(pat) ? hits.slice(-1) : hits).slice(0, 12)) out.push(f);
    }
  }
  for (const k of ['PROJECT_FOCUS_FILES', 'FACT_SOURCE_FILES']) {
    const v = env[k];
    const list = Array.isArray(v) ? v : (typeof v === 'string' && v.trim() ? [v] : []);
    for (const f of list.slice(0, 10)) if (typeof f === 'string') out.push(path.resolve(expand(f)));
  }
  return [...new Set(out)];
}
...
reg.register({
  ...
  available(env) {
    return contextFiles(env || {}).length ? { ok: true }
      : { ok: false, reason: '未接：设置里还没配本机项目资料（PROJECT_CONTEXT_DIR / PROJECT_FOCUS_FILES / FACT_SOURCE_FILES）' };
  },
  run(args, ctx) {
    const ts = terms(args.query);
    if (!ts.length) return { ok: false, error: '关键词里没有可检索的词' };
...
function rosterLines(env, names) {
  const f = String((env || {}).TEAM_MEMBERS_FILE || '').trim();
  if (!f) return [];
  let text = '';
  try { text = fs.readFileSync(path.resolve(expand(f)), 'utf8').slice(0, 200000); } catch (e) { return []; }
  const stamp = (() => { try { return new Date(fs.statSync(path.resolve(expand(f))).mtimeMs).toISOString(); } catch (e) { return ''; } })();
  const out = [];
  for (const n of names) {
    const low = String(n).toLowerCase();
    for (const line of text.split('\n')) {
      if (!line.trim() || !line.toLowerCase().includes(low)) continue;
      out.push({ query: n, text: clip(line.trim(), 300), from: 'roster', ref: 'file:' + path.resolve(expand(f)) + '@' + stamp, source: 'local:file', at: stamp });
      break;
    }
  }
  return out;
...
module.exports = { terms, loadMeetings, contextFiles, globIn };
