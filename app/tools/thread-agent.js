...
// Slack 走 tht-slack（app/tools/slack-cli.js 的壳，card-thread 把它写到 <数据目录>/state/bin 并加进 PATH），口令从本机 settings.json 读，不经模型、不进命令行参数。
...
// 三种要确认的动作，各自一小集写工具（Bash 前缀 + lark-mcp 的精确工具名）
...
// agent 的提问在问哪种动作：按关键词归类。归不进任何一类的问句不算待确认动作，用户回「是」也放不开写工具。
const ACTION_RE = {
...
function pendingActionOf(replyText) {
  ...
  if (!/[?？]/.test(t)) return '';
  return ACTIONS.filter(a => ACTION_RE[a].test(t)).join(',');
}
function writeToolsFor(action, { mcp = false } = {}) {
  const acts = String(action || '').split(',').filter(a => WRITE_BY_ACTION[a]);
  return [...new Set(acts.flatMap(a => [...WRITE_BY_ACTION[a].bash, ...(mcp ? WRITE_BY_ACTION[a].mcp : [])]))];
}

const CHEAT_SHEET = [
  '工具只有 Bash 里的 lark-cli（已用 Aaron 本人身份登录，都加 --as user，输出是 JSON）和 tht-slack（Slack，口令已在本机配置里，命令里不用填）。用法速查：',
...
module.exports = { READ_TOOLS, LARK_READ_TOOLS, SLACK_READ_TOOLS, WRITE_TOOLS, WRITE_BY_ACTION, ACTIONS, MCP_READ_TOOLS, MCP_WRITE_TOOLS, LARK_MCP_TOOL_IDS, pendingActionOf, writeToolsFor, CHEAT_SHEET, SLACK_CHEAT_SHEET };
