...
class Hub{
 ...
 async resolveTitles(max=25){
  const list=this.data.sources.filter(s=>s.url&&!s.titleEdited&&(s.titleState==='unread'||missingTitle(s.title))).slice(0,max);
  ...
  for(const s of list){
   ...
   else if(!isUnread(s.title)){s.title=unreadTitle(s.url);s.titleState='unread';}
