...
async function generate({ dir, sessionId, enhanced, env = {}, dataDir, attendees = [], log = () => {}, at = new Date() }) {
  ...
  const warnings = [];
  const out = { sessionId: String(sessionId), generatedAt: now(), status: 'done', classifiedBy: 'rules', cards: [], thinking: {}, risks: [], warnings };
...
async function resolveIds(names, env, execImpl, log) {
  ...
  const r = await runCli(execImpl, ['contact', '+search-user', '--queries', list.join(','), '--as', 'user', '--format', 'json'], log);
  if (!r.ok) return { ids: [], missing: list };
  const users = [];
  const walk = v => { if (!v || typeof v !== 'object') return; if (typeof v.open_id === 'string' && v.open_id) users.push({ id: v.open_id, name: String(v.name || v.en_name || '') }); for (const x of Object.values(v)) walk(x); };
  walk(r.json);
  const ids = [], missing = [];
  for (const n of list) {
    const hit = users.find(u => norm(u.name) === norm(n)) || (users.length === list.length ? users[list.indexOf(n)] : null);
