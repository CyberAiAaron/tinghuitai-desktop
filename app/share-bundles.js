...
module.exports=function({settings}){
 ...
 function work(key,kind){const flag=kind==='lark'?'larkStatus':'status';if(running.has(key))return;running.add(key);const j=read(key);j[flag]='running';delete j.error;write(key,j);const child=spawn('python3',[path.join(__dirname,'share-bundle.py'),file(key),kind],{env:{...process.env,THT_DATA_DIR:settings.dataDir},stdio:'ignore'});const finish=()=>{running.delete(key);const j=read(key);if(j[flag]==='running'){j[flag]='error';j.error='处理已中断，请重试';write(key,j);}};child.on('error',finish);child.on('exit',finish);}
