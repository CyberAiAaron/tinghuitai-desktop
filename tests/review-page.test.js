...
test('会中议题树跟着归档落盘：checkpoint、归档会话、崩溃恢复三处都带上',()=>{
  ...
  assert.match(serverJs,/if\(msg\.outline&&role==='speaker'&&!isView\)session\.setOutline\(msg\.outline\)/);});   // end 帧带来的也只收 speaker、非旁观
...
test('会中议题摘要：合并进来的重述不单独占一行，起止按这一组的要点算，段落 id 带过去',()=>{
  ...
  assert.match(html,/try\{outline=outlineDigest\(cur\)\|\|undefined;\}catch\(e\)\{\}/);   // end 帧也带一份，最后一轮不会掉；摘要算不出来不能拦住 end 帧
...
test('R3 显示侧：转写缺口（gapWarning）在回看页和归档列表都显示，和说话人存疑（speakerWarning）并存不互相顶掉',()=>{
  ...
  // 回看页脚本改了，缓存串要跟着加一
  assert.match(archiveHtml,/archive\.js\?v=17"/,'archive.js 改了，archive.html 的 ?v= 要加一');
});
