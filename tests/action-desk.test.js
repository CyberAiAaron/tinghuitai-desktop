...
// 真实 lark-cli（2026-09-22 实测）回的是扁平 users[]：{open_id, localized_name, matched_query}，没有 name 字段。
// 搜「Calvin」会同时出来 Calvin Gao 和 Calvin Chao——邀请发错人是真外发，重名必须落进 missing，不许按位置猜。
test('找人：按真实返回的字段对名字，重名不猜', async () => {
  const out = JSON.stringify({ ok: true, data: { users: [
    { open_id: 'ou_cary', localized_name: 'Cary Luo', matched_query: 'Cary Luo' },
    { open_id: 'ou_cg', localized_name: 'Calvin Gao', matched_query: 'Calvin' },
    { open_id: 'ou_cc', localized_name: 'Calvin Chao', matched_query: 'Calvin' },
    { open_id: 'ou_abel', localized_name: 'Abel Mei', matched_query: 'Abel' }] } });
  let seen = null;
  const exec = (cli, args, opts, cb) => { seen = args; cb(null, out, ''); };
  const r = await actions.resolveIds(['Cary Luo', 'Calvin', 'Abel', '新宇'], {}, exec, () => {});
  assert.deepEqual(r.ids, ['ou_cary', 'ou_abel']);          // 全等的认；关键词只搜出一个人的也认
  assert.deepEqual(r.missing, ['Calvin', '新宇']);            // 两个 Calvin 不猜；没搜到的不猜
  assert.ok(seen.includes('--exclude-external-users'));      // 不把外部联系人拉进会
  const r2 = await actions.resolveIds(['Calvin Gao'], {}, exec, () => {});
  assert.deepEqual(r2.ids, ['ou_cg']);
});
