...
test('候选分组和行渲染：按会分组、每条给得出回原句的路', ()=>{
 const c={db:{tasks:[
   ...
   {id:'t2',text:'S2 负责乙',owner:'S2',due:'',bucket:'candidate',sourceIds:['s1']},            // 老数据：没有时间戳
  ...
  sources:[{id:'s1',title:'周会',kind:'meeting',sessionId:'sess-1',date:'2026-09-20',speakerNames:{'2':'Cary Luo'}},
           {id:'s2',title:'产品总纲',kind:'document',url:'https://x/doc'}]},
  esc:s=>String(s??''),day:d=>d,bucket:w=>w.bucket||'mine'};
 vm.createContext(c);
 vm.runInContext(work.slice(work.indexOf('function applySpeakerNames('),work.indexOf('const TRIAGE_DONE')),c);
 ...
 assert.match(html,/Cary Luo 负责乙/,'认过人的会，候选待办里的 S 编号显示成人名');
 assert.equal(html.includes('S2'),false);
});
