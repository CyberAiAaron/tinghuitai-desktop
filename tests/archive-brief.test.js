...
test('项目背景：只读背景目录里点到的文件，目录外的通配不带进来；点评调用不带任何工具',()=>{
  ...
  const out=py(`import pathlib\nmp.read=lambda p,d=None:{'PROJECT_CONTEXT_FILES':['kb_reorg/*.md','../*','/etc/hosts']}\nprint(json.dumps(mp.load_context(pathlib.Path(${JSON.stringify(ctx)}))))`);
  assert.match(out,/=== 文件：kb_reorg\/02_总纲\.md ===\n总纲正文/);assert.doesNotMatch(out,/localhost|规则/);
