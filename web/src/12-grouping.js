  ...
  // 会中排好的议题树要送一份到 Mac：会后回看页照这套划分渲染，不另起一套议题。
  // 送的是算好的摘要（标题、结论、起止、这一组的要点带 segId），不是 keys 原文——服务端不用再回表查一遍。
  function outlineDigest(sess){
    const groups=(sess&&sess.hlGroups&&sess.hlGroups.groups)||[];
    if(groups.length<2)return null;
    const byText=new Map();for(const x of (sess.highlights||[]))if(!byText.has(x.text))byText.set(x.text,x);
    const out=[];
    for(const g of groups){
      const list=(g.keys||[]).map(k=>byText.get(k)).filter(Boolean);
      if(!list.length)continue;
      const merged=g.merged||{},hidden=new Set();
      for(const [keep,from] of Object.entries(merged))for(const t of from)if(hlKey(t)!==hlKey(keep))hidden.add(hlKey(t));
      const main=list.filter(x=>!hidden.has(hlKey(x.text)));
      const ats=list.map(x=>atOf(sess,x)).filter(v=>v>0);
      out.push({title:String(g.title||'').slice(0,120),summary:String(g.summary||'').slice(0,600),
        status:g.status==='unresolved'?'unresolved':'settled',
        from:ats.length?Math.min(...ats):0,to:ats.length?Math.max(...ats):0,
        points:(main.length?main:list).slice(0,12).map(x=>({text:String(x.text||'').slice(0,200),at:atOf(sess,x)||0,seg:segOf(x)}))});
    }
    return out.length>=2?out:null;
  }
  function sendOutline(sess){
    try{
      if(cur!==sess||!asrWs||asrWs.readyState!==1)return;
      const groups=outlineDigest(sess);
      if(groups)asrWs.send(JSON.stringify({type:'outline',groups}));
    }catch(e){console.debug('[outline] 没送上去',e&&e.message);}
  }
  ...
  function scheduleGrouping(sess, items){
    ...
    fetch(relayBase()+'/hub/llm?token='+encodeURIComponent(cfg.relayToken||''),{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({prompt,tier:'full',purpose:'auto',sessionId:'outline:'+sess.id}),signal:AbortSignal.timeout(120000)})
      ...
      .then(j=>{
        ...
        persist();sendOutline(sess);if(cur===sess)render();
