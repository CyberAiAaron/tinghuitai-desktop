  ...
  function render(){
    ...
    if (nTr !== sigTr) {
      sigTr = nTr;
      el.tr.innerHTML = tr.length ? tr.map((x,i)=>`<p><time>${hms(x.at)}</time>${spkChip(x.spk)}<span role="button" tabindex="0" data-fix="tr" data-key="${i}" title="点击修改逐字稿">${repeatedASR(x.text)?'<span class="asr-anomaly"><button type="button" class="btn sm" data-asr-expand>异常重复转写 · 展开原文</button><span hidden>'+esc(tt(x.text))+'</span></span>':esc(tt(x.text))}${!repeatedASR(x.text)&&tt(x.text)!==x.text?`<small style="display:block;color:var(--muted)">${esc(tt(x.text))}</small>`:''}${x.memo?`<small class="memo" style="display:block;color:var(--muted)">📝 ${esc(x.memo)}</small>`:''}</span></p>`).join('') + (interim?`<p><time>…</time><span class="spk s0">·</span><span class="interim">${esc(interim)}</span></p>`:'') : `<div class="empty">${running?(T('e_tr_live')||'正在听……'):(cur.transcript.length?'':(T('e_tr_none')||'这场还没有内容。'))}</div>`;
    ...
    if (nCk !== sigCk) {
      const first = sigCk === '';
      sigCk = nCk;
      keepScroll(el.ck, () => { el.ck.innerHTML = cks.length ? cks.map((x,i)=>`<div class="card ck${(!first&&i===cks.length-1)?' fresh':''}${x.pendingFix?' pending':''}" data-fix="ck" data-key="${esc(tt(x.claim))}" title="点一下可以改或删"><span class="k">${x.at?hms(x.at).slice(0,5):''}</span><div>${esc(tt(x.claim))}<div class="v"><span class="verdict ${x.verdict}">${x.verdict==='true'?(T('v_true')||'大概率对'):x.verdict==='false'?(T('v_false')||'可能有误'):(T('v_unsure')||'拿不准')}</span>${esc(tt(x.note||''))}</div>${x.memo?`<div class="v memo">📝 ${esc(x.memo)}</div>`:''}</div></div>`).join('') : `<div class="empty">${T('e_ck')||'对话里提到的公司、数字、事件会标出来并给初步判断。'}</div>`; }); if (first) el.ck.scrollTop = el.ck.scrollHeight;
    }
