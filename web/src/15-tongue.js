  ...
  function mergeInsights(list, incoming, at){
    ...
    for(const x of (incoming||[])){
      ...
      if(i>=0){ const old=list[i]; if(old.rating) row.rating=old.rating; if(old.comment) row.comment=old.comment; for(const k of ['actionState','correction','quote','doc','task','onePager']) if(old[k]!==undefined&&x[k]===undefined) row[k]=old[k]; list[i]=row; }
      else list.push(row);
      for(const k of ['actionState','correction','quote','doc','task','onePager']) if(x[k]!==undefined&&row[k]===undefined) row[k]=x[k];   // 服务端快照里带执行态和产物的，接回来
      n++;
    }
    return n;
  }
